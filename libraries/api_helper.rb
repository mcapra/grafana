module GrafanaCookbook
  module ApiHelper
    ...
    def login(host, port, user, password)
      ...
      regex = '(\w+)=([a-f0-9]{32});' 
      {'login_cookie_name' => cookie[/#{regex}/, 1], 'session_id' => cookie[/#{regex}/, 2]}
    rescue BackendError
      nil
    end
    ...
    def do_request(grafana_options, payload = nil)
      ...
      if grafana_options[:auth_proxy_header]
        ...
        session = login(grafana_options[:host], grafana_options[:port], grafana_options[:user], grafana_options[:password])
        request.add_field('Cookie', "session=#{grafana_options[:user]}; #{session['login_cookie_name']}=#{session['session_id']};")
      end
      request.add_field('Content-Type', 'application/json;charset=utf-8;')
